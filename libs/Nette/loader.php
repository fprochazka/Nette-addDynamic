...
function
setContext(Nette\DI\IContainer$context){$this->context=$context;if($this->invalidLinkMode===NULL){$this->invalidLinkMode=empty($context->parameters['productionMode'])?self::INVALID_LINK_WARNING:self::INVALID_LINK_SILENT;}}final
function
...
class
Framework{const
NAME='Nette Framework',VERSION='2.1-dev',REVISION='064ddb0 released on 2012-02-29';public
...
function
setupSecurity(ContainerBuilder$container,array$config){$container->addDefinition($this->prefix('userStorage'))->setClass('Nette\Http\UserStorage');$user=$container->addDefinition('user')->setClass('Nette\Security\User');if(!$container->parameters['productionMode']&&$config['debugger']){$user->addSetup('Nette\Diagnostics\Debugger::$bar->addPanel(?)',array(new
...
FALSE;}if($key===NULL){$this->accessed='';}elseif(!is_string($this->accessed)){$this->accessed[$key]=$cache;}if($cache&&!$this->select&&$this->prevAccessed&&($key===NULL||!isset($this->prevAccessed[$key]))){$this->prevAccessed='';$this->rows=NULL;return
TRUE;}return
FALSE;}function
insert($data){if($data
instanceof
...
as$key=>$row){$ref=&$referencing[$row[$this->column]];$skip=&$offset[$row[$this->column]];if($limit===NULL||$rows<=1||(count($ref)<$limit&&$skip>=$this->offset)){$ref[$key]=$row;}else{unset($this->rows[$key]);}$skip++;unset($ref,$skip);}}$this->data=&$referencing[$this->active];if($this->data===NULL){$this->data=array();}else{foreach($this->data
as$row){$row->getTable()->active=$this->active;}reset($this->data);}}}}namespace Nette\DI{use
Nette;class
Container
extends
...
HtmlNode($token->name);$htmlNode->isEmpty=in_array($this->contentType,array(self::CONTENT_HTML,self::CONTENT_XHTML))&&isset(Nette\Utils\Html::$emptyElements[strtolower($token->name)]);$htmlNode->offset=strlen($this->output);$this->setContext(self::CONTEXT_UNQUOTED);}$this->output.=$token->text;}private
function
...
expandMacro($name,$args,$modifiers=NULL,HtmlNode$htmlNode=NULL,$prefix=NULL){if(empty($this->macros[$name])){$js=$this->htmlNodes&&strtolower(end($this->htmlNodes)->name)==='script';throw
new
...
macroUse(MacroNode$node,PhpWriter$writer){call_user_func(array($node->tokenizer->fetchWord(),'install'),$this->getCompiler())->initialize();}function
